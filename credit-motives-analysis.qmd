...
Here we drop the folks who wrote that the names were suspicious in the open-end. Participants were not required to answer this question; many left it blank, and the question was vague ("do you have any thoughts or comments about the study? You may leave this blank if none.") But, it's a start, and we do this better in Study 2.
...
credit_no_drop <- credit_clean # save an old version
...
Participants may have understood what the study was about due to the within-subjects nature (i.e., seeing that the names in the vignettes conveyed different races and genders), even if they did not say so in the (optional) open-end item. One thing we could do is analyze just the FIRST vignette they did, before they saw that names were being manipulated. First, we need to read the vignette they got first:
...
MLMs instead of ANOVA. Also controls for what vignette they did, and their own gender and race. None of the interactions are significant, as in the previous model.

```{r}
mlm_1 <- lmer(mean_alt ~ 
                       actor_female*actor_black*condition + 
                       participant_white_only*participant_female + 
                       as.factor(vignette) +
...
repeat analyses with people not dropped. First, need to make a long version:

```{r}

credit_long_no_drop <- credit_no_drop %>%
  pivot_longer(
    cols = starts_with("v"),
    names_to = c("vignette", ".value"),
    names_pattern = "v(\\d+)_(.*)"
...
credit_long_no_drop <- credit_long_no_drop %>% 
  mutate(
    person_rec = person - 5,  #recode these from 1-9 scale to -4 to 4 scale
    charmotive_rec = charmotive - 5,
    action_rec = action - 5,
...
Then, re-do our anova but with all participants included:

```{r}


anova_model_no_drop <- aov_ez(
  id = "ResponseId",
  dv = "mean_alt",
  data = credit_long_no_drop,
  between = "condition", # varied across participants
  within = c("actor_female", "actor_black"), #varied within participants
  type = 3
)

nice(anova_model_no_drop, es = "pes") # using partial eta squared since we have many interactions. 
```

We should also repeat our anova but with participant categories:
...
anova_model_demos <- aov_ez(
  ...
  dv = "mean_alt",
  data = credit_long,
  between = c("condition", "participant_white", "participant_female"), # varied across participants
  within = c("actor_female", "actor_black"), #varied within participants
  type = 3
)
...
There is an interaction between participant demos (both White and female) and female actors (and condition).Probing that: basically, two diffs in the difference between neutral vs self interested actors, rather than penalties or credits.

```{r}
emm <- emmeans(anova_model_demos, 
        ~ condition | participant_white * participant_female * actor_female)
