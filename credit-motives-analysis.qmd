---
title: "Study 1 - Do lay theories of altruism apply equally to all actors?"
...
Carlson and Zaki dropped participants who read each vignette in 1.5 sec or less. 
Unfortunately, I did not put a time tracker on each individual vignette screen. 
But, we can see how much time people spent on the vignettes total, and average 
this per vignette. First we take the duration in secs they spent on the entire 
study (was in the fourth column; I relabelled as `duration`) and subtract the 
time they spent on the consent form, which does have a time tracker on it (10th 
column, I relabelled as `start`). This leaves the time they spent on everything 
*but* the consent screen. Then we get the z-scores of those and finally call 
them `outlier` if they are more than 3 z-scores different.
...
credit_clean <- credit %>% 
  ...
  mutate(
    across(c(v1_person:v8_others), as.numeric)
      ...
      select(ResponseId, participant_gender, participant_hispanic, participant_race, v1_gender, v1_race, v2_gender, v2_race, v3_gender, v3_race, v4_gender, v4_race, v5_gender, v5_race, v6_gender, v6_race, v7_gender, v7_race, v8_gender, v8_race, help, motives, v1_person:v8_others, v1_motive:v8_motive, FL_655_DO, open_end_flag
  )
```

Here we drop the folks who wrote that the names were 
suspicious in the open-end. Participants were not required to answer this 
question; many left it blank, and the question was vague ("do you have any
thoughts or comments about the study? You may leave this blank if none.")
But, it's a start, and we do this better in Study 2. 

```{r}

credit_clean %>%
  filter(open_end_flag == 1) %>%
  tally() # 11 people said something about the names

credit_clean <- credit_clean%>% 
  filter(is.na(open_end_flag)
)
```

How many Ps do we have now?
...
Now we'll make the dataset long. We take the `v1_gender`, `v1_race`...etc 
variables and convert them to new variables `actor_female` and `actor_race` 
for each vignette. I also created some updated condition variables that are 
mutually exclusive (compared to the dummy coded version) in the data where help 
== 1 refers to both the help and help-with-motives condition).
...
Cronbach alpha on the four items Ps answered after each vignette, separated out 
for each vignette so that we aren't getting a high alpha due to high 
within-rater reliability. We compare the alpha for the three-item (just the 
three items from CZ) and four-item (the fourth we added) scales. Both look good 
and very similar so we'll retain the four-item scale. I also created average 
values across the vignettes to report that too.
...
Because the items are very reliable, I'm averaging them into a new DV called 
`mean_alt`. Before I do that I also rescale the items so the midpoint of the 
scale is 0 (-4 to 4) rather than 5 (1-9). Creating another condition variable 
here too.
...
credit_long <- credit_long %>% 
  mutate(
    ...
    participant_white = ifelse(str_detect(participant_race, "White"), 1, 0), 
    participant_white_only = ifelse(participant_race == "White/Caucasian", 1, 0), # this version only selects participants who indicated White AND NO OTHER RACE as a stricter form
...
fig1 <- ggplot(emm_df, aes(x = reorder(condition, emmean), y = emmean)) +
...
print(old_fig2)
ggsave("old_fig2.pdf", old_fig2, width = 7, height = 5, bg = "white")
ggsave("old_fig2.png", old_fig2, width = 7, height = 5, bg = "white")

```


## New analysis for R&R

First, we want to update Figure 2 to show the data as a whole, but then also panels for just White participants, just men, and just women. This could visualize how the results may differ based on participant characteristics. 
...
make_panel_df <- function(dat, panel_label) {
  ...
  mod <- aov_ez(
    ...
    data = dat,
    between = "condition",
    within = c("actor_female", "actor_black"),
    type = 3
  )
...
diffs_white <- make_panel_df(
  credit_long %>% filter(participant_race == "White/Caucasian"),
  "White participants only"
)

diffs_men <- make_panel_df(
  credit_long %>% filter(participant_gender == "Man"),
  "Men only"
)

diffs_women <- make_panel_df(
  credit_long %>% filter(participant_gender == "Woman"),
  "Women only"
)
...
print(fig2)
ggsave("fig2.pdf", fig2, width = 10, height = 7, bg = "white")
ggsave("fig2.png", fig2, width = 10, height = 7, bg = "white")
```


Participants may have understood what the study was about due to the within-subjects nature (i.e., seeing that the names in the vignettes conveyed different races and genders). One thing we could do is analyze just the FIRST vignette they did, before they saw that names were being manipulated. First, we need to read the vignette they got first:

```{r}
credit_long %>% mutate(
  first_vignette = str_extract(FL_655_DO, "\\d+") )  %>% 
   select(first_vignette, FL_655_DO) # ran this first to make sure it's right

credit_long <- credit_long %>% mutate(
  first_vignette = str_extract(FL_655_DO, "\\d+") ) 
```

Then make a data frame that contains only the vignettes that were their first vignette:

```{r}
credit_first_vignette <- credit_long %>% 
  filter(vignette == first_vignette)
```

Now let's repeat the ANOVA with the first vignettes only:

```{r}
anova_model_first <- aov_ez(
  id = "ResponseId",
  dv = "mean_alt",
  data = credit_first_vignette,
  between = c("condition", "actor_female", "actor_black"), #varied between participants for "first vignette only", rather than within participants
  type = 3
)

nice(anova_model_first, es = "pes") 
```

Here are the simple effects:

```{r}
emm <- emmeans(anova_model_first, ~ actor_female * actor_black * condition)
emm
```
...
Let's split these out by motive, using an alt condition variable I created above, which looks at material vs. social separately:
...
Getting bootstrapped CIs for this model too:

```{r}
broom.mixed::tidy(mlm_2)
set.seed(20251219)
confint(mlm_2, method = "boot", nsim = 1000, level = 0.95)
```
